...
const CuentasDeMayorPorTerceros = require('../../models/reports/CuentasDeMayorPorTercerosModel');
...
exports.loadAsyncy = async (req, res) => {
  try {
    ...
    objectReportResume.code = 'CUENTASMAYORPORTERCEROSTM';
    ...
    await workbook.xlsx.readFile(pathx).then(function() {
      ...
      workSheet.eachRow(function(row, rowNumber) {
        ...
        if (count === 0) {
          ...
          if (
            fileTitle !== constants.CUENTAS_MAYOR_POR_TERCEROS_TEMPLATE_TITLE
          ) {
        ...
        if (count > constants.CUENTAS_MAYOR_POR_TERCEROS_ROW_INIT) {
          const rowInsert = {
            idCuentaDeMayor: currRow.getCell(2).value,
            nombreCuentaDeMayor: currRow.getCell(3).value,
            idClienteProveedorContrapartida: currRow.getCell(4).value,
            nombreClienteProveedorContrapartida: currRow.getCell(5).value,
            asientoContable: currRow.getCell(6).value,
            asientoContableDeAnulacion: currRow.getCell(7).value,
            idCentroCostos: currRow.getCell(8).value,
            nombreCentroCostos: currRow.getCell(9).value,
            fechaContabilizacion: customValidator.dateFromString(
              currRow.getCell(10).value
            ),
            idDocumentoOriginal: currRow.getCell(11).value,
            periodoContable: currRow.getCell(12).value,
            idSocioComercial: currRow.getCell(13).value,
            nombreSocioComercial: currRow.getCell(14).value,
            textoCabeceraAsientoContable: currRow.getCell(15).value,
            textoPosicionAsientoContable: currRow.getCell(16).value,
            saldoInicialMonedaEmpresa: currRow.getCell(17).value,
            importeDebeMonedaEmpresa: currRow.getCell(18).value,
            importeHaberMonedaEmpresa: currRow.getCell(19).value,
            saldoMonedaEmpresa: currRow.getCell(20).value
          };
    ...
    await reportFunctionsUpdate.updateReportUploader(objectReportResume);
    const countDB = await CuentasDeMayorPorTerceros.countDocuments();
    await CuentasDeMayorPorTerceros.collection
...
exports.delete = async (req, res) => {
  try {
    await CuentasDeMayorPorTerceros.deleteMany();
    // Defino objeto y variables estandar para el resumen de la carga
    const objectReportResume = {};
    objectReportResume.code = 'CUENTASMAYORPORTERCEROSTM';
...
exports.getAll = async req => {
  ...
  features = new APIFeatures(
    CuentasDeMayorPorTerceros.find()
  ...
  const featuresCounter = new APIFeatures(
    CuentasDeMayorPorTerceros.countDocuments(),
    req.query
  );
  ...
  try {
    ...
    if (!reportInfo) {
      reportInfo = await reportDownloaderService.createReportByUser({
        name: 'Cuentas de Mayor por Terceros',
        code: objectReportResume.code,
        generatorUserId: objectReportResume.generatorUserId,
        reportType: objectReportResume.reportType
      });
    ...
    const nameFile = 'CUENTAS_DE_MAYOR_POR_TERCEROS';
    const pathTmp = path.resolve(__dirname, '../../resources/uploads/');
