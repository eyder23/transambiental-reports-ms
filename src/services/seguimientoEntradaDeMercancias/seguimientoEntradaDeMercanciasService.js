...
const SeguimientoEntradaDeMercancias = require('../../models/reports/SeguimientoEntradaDeMercanciasModel');
...
exports.loadAsyncy = async (req, res) => {
  try {
    ...
    objectReportResume.code = 'SEGUIMIENTOENTRADADEMERCANCIASTM';
    ...
    await workbook.xlsx.readFile(pathx).then(function() {
      ...
      workSheet.eachRow(function(row, rowNumber) {
        ...
        if (count === 0) {
          ...
          if (
            fileTitle !==
            constants.SEGUIMIENTO_ENTRADA_DE_MERCANCIAS_TEMPLATE_TITLE
          ) {
        ...
        if (count > constants.SEGUIMIENTO_ENTRADA_DE_MERCANCIAS_ROW_INIT) {
          const rowInsert = {
            estadoEntradaMercanciasServicios: currRow.getCell(2).value,
            idProveedor: currRow.getCell(3).value,
            nombreProveedor: currRow.getCell(4).value,
            idProducto: currRow.getCell(5).value,
            nombreProducto: currRow.getCell(6).value,
            idEntradaMercanciasServicios: currRow.getCell(7).value,
            idPosicionEntradaMercanciasServicios: currRow.getCell(8).value,
            idPedidoCompra: currRow.getCell(9).value,
            cantidad: currRow.getCell(10).value,
            valorNeto: currRow.getCell(11).value,
            valorNetoMonedaEmpresa: currRow.getCell(12).value,
            precio: currRow.getCell(13).value
          };
    ...
    await reportFunctionsUpdate.updateReportUploader(objectReportResume);
    const countDB = await SeguimientoEntradaDeMercancias.countDocuments();
    await SeguimientoEntradaDeMercancias.collection
...
exports.delete = async (req, res) => {
  try {
    await SeguimientoEntradaDeMercancias.deleteMany();
    // Defino objeto y variables estandar para el resumen de la carga
    const objectReportResume = {};
    objectReportResume.code = 'SEGUIMIENTOENTRADADEMERCANCIASTM';
...
exports.getAll = async req => {
  ...
  features = new APIFeatures(
    SeguimientoEntradaDeMercancias.find()
      // .sort({ creadoEl: 1 })
      .lean(),
    req.query
  )
  ...
  const featuresCounter = new APIFeatures(
    SeguimientoEntradaDeMercancias.countDocuments(),
    req.query
  );
...
exports.getAllAdvancedFilter = async req => {
  ...
  if (!isToDownload) {
    ...
    features = new APIFeatures(
      ...
      .findByExact('idProveedor', req.body.idProveedor)
      .findByExact('nombreProveedor', req.body.nombreProveedor)
      .findByExact(
        'idEntradaMercanciasServicios',
        req.body.idEntradaMercanciasServicios
      )
      .findByExact('idPedidoCompra', req.body.idPedidoCompra)
      .limitFields()
  ...
  try {
    ...
    if (!reportInfo) {
      reportInfo = await reportDownloaderService.createReportByUser({
        name: 'Seguimiento Entrada de Mercancias',
        code: objectReportResume.code,
        generatorUserId: objectReportResume.generatorUserId,
        reportType: objectReportResume.reportType
      });
    ...
    const nameFile = 'SEGUIMIENTO_ENTRADA_DE_MERCANCIAS';
    const pathTmp = path.resolve(__dirname, '../../resources/uploads/');
    ...
    const csvWriter = createCsvWriter({
      ...
      header: [
        {
          id: 'estadoEntradaMercanciasServicios',
          title: 'Estado Entrada Mercancias y Servicios'
        },
        {
          id: 'idProveedor',
          title: 'ID Proveedor'
        ...
        {
          id: 'idEntradaMercanciasServicios',
          title: 'Id Entrada Mercancias y Servicios'
        },
        {
          id: 'idPosicionEntradaMercanciasServicios',
          title: 'Id Posicion Entrada Mercancias y Servicios'
        },
        {
          id: 'idPedidoCompra',
        ...
        {
          id: 'valorNetoMonedaEmpresa',
          title: 'Valor Neto Moneda Empresa'
        },
        {
          id: 'precio',
          title: 'Precio'
    ...
    reportData.forEach(function(cursor, index, object) {
      ...
      cursor.valorNeto = customValidator.numberToStringDecimal(
        cursor.valorNeto
      );
      cursor.valorNetoMonedaEmpresa = customValidator.numberToStringDecimal(
        cursor.valorNetoMonedaEmpresa
      );
      cursor.precio = customValidator.numberToStringDecimal(cursor.precio);
    });
