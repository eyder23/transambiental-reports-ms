...
const BalanceDeComprobacion = require('../../models/reports/BalanceDeComprobacionModel');
...
exports.loadAsyncy = async (req, res) => {
  try {
    ...
    objectReportResume.code = 'BALANCEDECOMPROBACIONTM';
    ...
    await workbook.xlsx.readFile(pathx).then(function() {
      ...
      workSheet.eachRow(function(row, rowNumber) {
        ...
        if (count === 2) {
          const fileTitle = currRow.getCell(1).value;
          if (fileTitle !== constants.BALANCE_DE_COMPROBACION_TEMPLATE_TITLE) {
        ...
        if (count > constants.BALANCE_DE_COMPROBACION_ROW_INIT) {
          const rowInsert = {
            partidaBalanceCuentaResultados: currRow.getCell(1).value,
            importeMonedaEmpresa: currRow.getCell(2).value,
            periodo: currRow.getCell(3).value.toString(),
            anio: currRow.getCell(4).value.toString()
          };
    ...
    await reportFunctionsUpdate.updateReportUploader(objectReportResume);
    const countDB = await BalanceDeComprobacion.countDocuments();
    await BalanceDeComprobacion.collection
...
exports.delete = async (req, res) => {
  try {
    await BalanceDeComprobacion.deleteMany();
    // Defino objeto y variables estandar para el resumen de la carga
    const objectReportResume = {};
    objectReportResume.code = 'BALANCEDECOMPROBACIONTM';
...
exports.getAll = async req => {
  ...
  const featuresCounter = new APIFeatures(
    BalanceDeComprobacion.countDocuments(),
    req.query
  );
  ...
  if (!isToDownload) {
    ...
    features = new APIFeatures(
      ...
      .findByExact(
        ...
        req.body.partidaBalanceCuentaResultados
      )
      .findByExactNoLike('periodo', req.body.periodo)
      .findByExact('anio', req.body.anio)
      .limitFields()
...
exports.sendReportToEmail = async (req, res) => {
  ...
  try {
    ...
    const nameFile = 'BALANCE_DE_COMPROBACION';
    const pathTmp = path.resolve(__dirname, '../../resources/uploads/');
    ...
    const csvWriter = createCsvWriter({
      ...
      header: [
        {
          id: 'partidaBalanceCuentaResultados',
          title: 'Partida de Balance y Cuenta de Resultados'
        },
        {
          id: 'importeMonedaEmpresa',
          title: 'Importe en Moneda de la Empresa'
        },
        {
          id: 'periodo',
          title: 'Periodo'
    ...
    reportData.forEach(function(cursor, index, object) {
      cursor.importeMonedaEmpresa = customValidator.numberToStringDecimal(
        cursor.importeMonedaEmpresa
      );
    });
