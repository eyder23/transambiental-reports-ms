...
const EstadoDeResultados = require('../../models/reports/EstadoDeResultadosModel');
...
exports.loadAsyncy = async (req, res) => {
  try {
    ...
    objectReportResume.code = 'ESTADODERESULTADOSTM';
    ...
    await workbook.xlsx.readFile(pathx).then(function() {
      ...
      workSheet.eachRow(function(row, rowNumber) {
        ...
        if (count === 2) {
          ...
          if (fileTitle !== constants.ESTADO_DE_RESULTADOS_TEMPLATE_TITLE) {
        ...
        if (count > constants.ESTADO_DE_RESULTADOS_ROW_INIT) {
          const rowInsert = {
    ...
    await reportFunctionsUpdate.updateReportUploader(objectReportResume);
    const countDB = await EstadoDeResultados.countDocuments();
    await EstadoDeResultados.collection
...
exports.delete = async (req, res) => {
  try {
    await EstadoDeResultados.deleteMany();
    // Defino objeto y variables estandar para el resumen de la carga
    const objectReportResume = {};
    objectReportResume.code = 'ESTADODERESULTADOSTM';
...
exports.getAll = async req => {
  ...
  const featuresCounter = new APIFeatures(
    EstadoDeResultados.countDocuments(),
    req.query
  );
...
exports.sendReportToEmail = async (req, res) => {
  ...
  try {
    ...
    const nameFile = 'ESTADO_DE_RESULTADOS';
    const pathTmp = path.resolve(__dirname, '../../resources/uploads/');
