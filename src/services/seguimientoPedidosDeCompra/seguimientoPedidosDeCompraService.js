...
const SeguimientoPedidosDeCompra = require('../../models/reports/SeguimientoPedidosDeCompraModel');
...
exports.loadAsyncy = async (req, res) => {
  try {
    ...
    objectReportResume.code = 'SEGUIMIENTOPEDIDOSDECOMPRATM';
    ...
    await workbook.xlsx.readFile(pathx).then(function() {
      ...
      workSheet.eachRow(function(row, rowNumber) {
        ...
        if (count === 0) {
          ...
          if (
            fileTitle !== constants.SEGUIMIENTO_PEDIDOS_DE_COMPRA_TEMPLATE_TITLE
          ) {
        ...
        if (count > constants.SEGUIMIENTO_PEDIDOS_DE_COMPRA_ROW_INIT) {
          const rowInsert = {
            estadoPedidoCompra: currRow.getCell(2).value,
            idPedidoCompra: currRow.getCell(3).value,
            posicionPedidoCompra: currRow.getCell(4).value,
            proveedor: currRow.getCell(5).value,
            nombreProveedor: currRow.getCell(6).value,
            producto: currRow.getCell(7).value,
            fechaCreacionPedido: customValidator.dateFromString(
              currRow.getCell(8).value
            ),
            fechaPedido: customValidator.dateFromString(
              currRow.getCell(9).value
            ),
            precioNeto: currRow.getCell(10).value,
            cantidadPedida: currRow.getCell(11).value,
            cantidadEntregada: currRow.getCell(12).value,
            cantidadFacturada: currRow.getCell(13).value,
            valorEntregado: currRow.getCell(14).value,
            valorEntregadoEnMonedaEmpresa: currRow.getCell(15).value,
            valorFacturado: currRow.getCell(16).value,
            valorFacturadoEnMonedaEmpresa: currRow.getCell(17).value
          };
    ...
    await reportFunctionsUpdate.updateReportUploader(objectReportResume);
    const countDB = await SeguimientoPedidosDeCompra.countDocuments();
    await SeguimientoPedidosDeCompra.collection
...
exports.delete = async (req, res) => {
  try {
    await SeguimientoPedidosDeCompra.deleteMany();
    // Defino objeto y variables estandar para el resumen de la carga
    const objectReportResume = {};
    objectReportResume.code = 'SEGUIMIENTOPEDIDOSDECOMPRATM';
...
exports.getAll = async req => {
  ...
  features = new APIFeatures(
    SeguimientoPedidosDeCompra.find()
      // .sort({ creadoEl: 1 })
      .lean(),
    req.query
  )
  ...
  const featuresCounter = new APIFeatures(
    SeguimientoPedidosDeCompra.countDocuments(),
    req.query
  );
  ...
  if (!isToDownload) {
    ...
    features = new APIFeatures(
      ...
      .filterTableAdvancedFilterDate(
        'fechaCreacionPedido',
        req.body.fechaCreacionPedidoInit,
        req.body.fechaCreacionPedidoEnd
      )
      .findByExactNoLike('idPedidoCompra', req.body.idPedidoCompra)
  ...
  try {
    ...
    if (!reportInfo) {
      reportInfo = await reportDownloaderService.createReportByUser({
        name: 'Seguimiento Pedidos de Compra',
        code: objectReportResume.code,
        generatorUserId: objectReportResume.generatorUserId,
        reportType: objectReportResume.reportType
      });
    ...
    const nameFile = 'SEGUIMIENTO_PEDIDOS_DE_COMPRA';
    const pathTmp = path.resolve(__dirname, '../../resources/uploads/');
    ...
    const csvWriter = createCsvWriter({
      ...
      header: [
        {
          id: 'estadoPedidoCompra',
          title: 'Estado Pedido Compra'
        },
        {
          id: 'idPedidoCompra',
        ...
        {
          id: 'posicionPedidoCompra',
          title: 'Posición Pedido Compra'
        },
        {
          id: 'proveedor',
          title: 'ID Proveedor'
        ...
        {
          id: 'fechaCreacionPedido',
          title: 'Fecha Creación Pedido'
        },
        {
          id: 'fechaPedido',
          title: 'Feha Pedido'
        ...
        {
          id: 'valorEntregadoEnMonedaEmpresa',
          title: 'Valor Entregado en Moneda Empresa'
        },
        {
          id: 'valorFacturado',
        ...
        {
          id: 'valorFacturadoEnMonedaEmpresa',
          title: 'Valor Facturado en Moneda Empresa'
        }
      ]
    });
    ...
    reportData.forEach(function(cursor, index, object) {
      cursor.fechaCreacionPedido = customValidator.stringFromDate(
        cursor.fechaCreacionPedido
      );
      cursor.fechaPedido = customValidator.stringFromDate(cursor.fechaPedido);

      cursor.precioNeto = customValidator.numberToStringDecimal(
        cursor.precioNeto
      );
      cursor.valorEntregado = customValidator.numberToStringDecimal(
        cursor.valorEntregado
      );
      cursor.valorEntregadoEnMonedaEmpresa = customValidator.numberToStringDecimal(
        cursor.valorEntregadoEnMonedaEmpresa
      );
      cursor.valorFacturado = customValidator.numberToStringDecimal(
        cursor.valorFacturado
      );
      cursor.valorFacturadoEnMonedaEmpresa = customValidator.numberToStringDecimal(
        cursor.valorFacturadoEnMonedaEmpresa
      );
    });
