...
const HistorialDeCuenta = require('../../models/reports/HistorialDeCuentaModel');
...
exports.loadAsyncy = async (req, res) => {
  try {
    ...
    objectReportResume.code = 'HISTORIALDECUENTATM';
    ...
    await workbook.xlsx.readFile(pathx).then(function() {
      ...
      workSheet.eachRow(function(row, rowNumber) {
        ...
        if (count === 0) {
          ...
          if (fileTitle !== constants.HISTORIAL_DE_CUENTA_TEMPLATE_TITLE) {
        ...
        if (count > constants.HISTORIAL_DE_CUENTA_ROW_INIT) {
          const rowInsert = {
            idCuentaDeMayor: currRow.getCell(2).value,
            nombreCuentaDeMayor: currRow.getCell(3).value,
            periodoAnioContable: currRow.getCell(4).value,
            fechaContabilizacion: customValidator.dateFromString(
              currRow.getCell(5).value
            ),
            asientoContable: currRow.getCell(6).value,
            idDocumentoOriginal: currRow.getCell(7).value,
            asientoContableDeAnulacion: currRow.getCell(8).value,
            idDocumentoOriginalExtra: currRow.getCell(9).value,
            idCuentaContraPartidaLibroMayor: currRow.getCell(10).value,
            nombreCuentaContraPartidaLibroMayor: currRow.getCell(11).value,
            idClienteProveedorContrapartida: currRow.getCell(12).value,
            nombreClienteProveedorContrapartida: currRow.getCell(13).value,
            idDocumentoOriginalDeReferencia: currRow.getCell(14).value,
            idDocumentoOriginalExtra2: currRow.getCell(15).value,
            idDocumentoOperativo: currRow.getCell(16).value,
            referenciaExternaDocumentoOriginal: currRow.getCell(17).value,
            idSocioComercial: currRow.getCell(18).value,
            nombreSocioComercial: currRow.getCell(19).value,
            textoCabeceraAsientoContable: currRow.getCell(20).value,
            textoPosicionAsientoContable: currRow.getCell(21).value,
            saldoInicialMonedaEmpresa: currRow.getCell(22).value,
            importeDebeMonedaEmpresa: currRow.getCell(23).value,
            importeHaberMonedaEmpresa: currRow.getCell(24).value,
            saldoMonedaEmpresa: currRow.getCell(25).value
          };
    ...
    await reportFunctionsUpdate.updateReportUploader(objectReportResume);
    const countDB = await HistorialDeCuenta.countDocuments();
    await HistorialDeCuenta.collection
...
exports.delete = async (req, res) => {
  try {
    await HistorialDeCuenta.deleteMany();
    // Defino objeto y variables estandar para el resumen de la carga
    const objectReportResume = {};
    objectReportResume.code = 'HISTORIALDECUENTATM';
...
exports.getAll = async req => {
  ...
  const featuresCounter = new APIFeatures(
    HistorialDeCuenta.countDocuments(),
    req.query
  );
...
exports.getAllAdvancedFilter = async req => {
  ...
  if (!isToDownload) {
    ...
    features = new APIFeatures(
      ...
      .findByExact('periodoAnioContable', req.body.periodoAnioContable)
      .findByExact(
        'idClienteProveedorContrapartida',
        req.body.idClienteProveedorContrapartida
      )
      .findByExact(
        'nombreClienteProveedorContrapartida',
        req.body.nombreClienteProveedorContrapartida
      )
      .findByExact('idSocioComercial', req.body.idSocioComercial)
...
exports.sendReportToEmail = async (req, res) => {
  ...
  try {
    ...
    const nameFile = 'HISTORIAL_DE_CUENTA';
    const pathTmp = path.resolve(__dirname, '../../resources/uploads/');
    ...
    const csvWriter = createCsvWriter({
      ...
      header: [
        ...
        {
          id: 'idDocumentoOriginalExtra',
          title: 'Id Documento original extra'
        },
        {
          id: 'idCuentaContraPartidaLibroMayor',
          title: 'Id cuenta contra partida libro mayor'
        },
        {
          id: 'nombreCuentaContraPartidaLibroMayor',
          title: 'Nombre cuenta contra partida libro mayor'
        },
        {
          id: 'idClienteProveedorContrapartida',
          title: 'Id cliente proveedor contrapartida'
        },
        {
          id: 'nombreClienteProveedorContrapartida',
          title: 'Nombre cliente proveedor contrapartida'
        },
        {
          id: 'idDocumentoOriginalDeReferencia',
          title: 'Id documento original de referencia'
        },
        {
          id: 'idDocumentoOriginalExtra2',
          title: 'Id documento original extra'
        },
        {
          id: 'idDocumentoOperativo',
          title: 'Id documento operativo'
        },
        {
          id: 'referenciaExternaDocumentoOriginal',
          title: 'Referencia externa documento original'
        },
        {
          id: 'idSocioComercial',
        ...
        {
          id: 'textoCabeceraAsientoContable',
          title: 'Texto cabecera asiento contable'
        },
        {
          id: 'textoPosicionAsientoContable',
          title: 'Texto posición asiento contable'
        },
        {
          id: 'saldoInicialMonedaEmpresa',
          title: 'Saldo inicial moneda empresa'
        },
        {
          id: 'importeDebeMonedaEmpresa',
          title: 'Importe debe moneda empresa'
        },
        {
          id: 'importeHaberMonedaEmpresa',
          title: 'Importe haber moneda empresa'
        },
        {
          id: 'saldoMonedaEmpresa',
          title: 'Saldo moneda empresa'
        }
      ]
    });
    ...
    reportData.forEach(function(cursor, index, object) {
      ...
      cursor.saldoInicialMonedaEmpresa = customValidator.numberToStringDecimal(
        cursor.saldoInicialMonedaEmpresa
      );
