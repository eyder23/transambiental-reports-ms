...
const InformeDiarioDeConfirmacion = require('../../models/reports/InformeDiarioDeConfirmacionModel');
...
exports.loadAsyncy = async (req, res) => {
  try {
    ...
    objectReportResume.code = 'INFORMEDIARIODECONFIRMACIONTM';
    ...
    await workbook.xlsx.readFile(pathx).then(function() {
      ...
      workSheet.eachRow(function(row, rowNumber) {
        ...
        if (count === 0) {
          ...
          if (
            fileTitle !==
            constants.INFORME_DIARIO_DE_CONFIRMACION_TEMPLATE_TITLE
          ) {
        ...
        if (count > constants.INFORME_DIARIO_DE_CONFIRMACION_ROW_INIT) {
          const rowInsert = {
            sede: currRow.getCell(2).value,
            producto: currRow.getCell(3).value,
            identificador: currRow.getCell(4).value,
            idStockIdentificado: currRow.getCell(5).value,
            idAreaLogistica: currRow.getCell(6).value,
            creadoEl: customValidator.dateFromString(currRow.getCell(7).value),
            tipoDocumentoConfirmacion: currRow.getCell(8).value,
            idConfirmacion: currRow.getCell(9).value,
            cantidadEfectiva: currRow.getCell(10).value
          };
    ...
    await reportFunctionsUpdate.updateReportUploader(objectReportResume);
    const countDB = await InformeDiarioDeConfirmacion.countDocuments();
    await InformeDiarioDeConfirmacion.collection
...
exports.delete = async (req, res) => {
  try {
    await InformeDiarioDeConfirmacion.deleteMany();
    // Defino objeto y variables estandar para el resumen de la carga
    const objectReportResume = {};
    objectReportResume.code = 'INFORMEDIARIODECONFIRMACIONTM';
...
exports.getAll = async req => {
  ...
  features = new APIFeatures(
    InformeDiarioDeConfirmacion.find()
      // .sort({ creadoEl: 1 })
      .lean(),
    req.query
  )
  ...
  const featuresCounter = new APIFeatures(
    InformeDiarioDeConfirmacion.countDocuments(),
    req.query
  );
  ...
  if (!isToDownload) {
    ...
    features = new APIFeatures(
      ...
      .filterTableAdvancedFilterDate(
        'creadoEl',
        req.body.creadoElInit,
        req.body.creadoElEnd
      )
      .findByExact('sede', req.body.sede)
      .findByExact('producto', req.body.producto)
      .findByExact('idAreaLogistica', req.body.idAreaLogistica)
      .findByExactNoLike('idConfirmacion', req.body.idConfirmacion)
      .findByExactNoLike('identificador', req.body.identificador)
      .limitFields()
  ...
  try {
    ...
    if (!reportInfo) {
      reportInfo = await reportDownloaderService.createReportByUser({
        name: 'Informe Diario de Confirmacion',
        code: objectReportResume.code,
        generatorUserId: objectReportResume.generatorUserId,
        reportType: objectReportResume.reportType
      });
    ...
    const nameFile = 'INFORME_DIARIO_DE_CONFIRMACION';
    const pathTmp = path.resolve(__dirname, '../../resources/uploads/');
    ...
    const csvWriter = createCsvWriter({
      ...
      header: [
        ...
        {
          id: 'idStockIdentificado',
          title: 'ID Stock Identificado'
        },
        {
          id: 'idAreaLogistica',
          title: 'ID Área Logistica'
        },
        {
          id: 'creadoEl',
          title: 'Creado El'
        ...
        {
          id: 'tipoDocumentoConfirmacion',
          title: 'Tipo Documento Confirmación'
        },
        {
          id: 'idConfirmacion',
          title: 'ID Confirmación'
        },
        {
          id: 'cantidadEfectiva',
          title: 'Cantidad Efectiva'
        }
      ]
    });
    ...
    reportData.forEach(function(cursor, index, object) {
      cursor.creadoEl = customValidator.stringFromDate(cursor.creadoEl);
    });
