...
const Proveedor = require('../../models/reports/ProveedorModel');
...
exports.loadAsyncy = async (req, res) => {
  try {
    ...
    objectReportResume.code = 'PROVEEDORTM';
    ...
    await workbook.xlsx.readFile(pathx).then(function() {
      ...
      workSheet.eachRow(function(row, rowNumber) {
        ...
        if (count > constants.SUPPLIERS_TEMPLATE_ROW_INIT) {
          const rowInsert = {
            proveedor: currRow.getCell(2).value,
    ...
    await reportFunctionsUpdate.updateReportUploader(objectReportResume);
    const countDB = await Proveedor.countDocuments();
    await Proveedor.collection
...
exports.getAllAdvancedFilter = async req => {
  ...
  if (!isToDownload) {
    ...
    features = new APIFeatures(
      ...
      .findByExact('nombre', req.body.nombre)
      .findByExact(
        'numeroIdentificacionFiscal',
        req.body.numeroIdentificacionFiscal
      )
      .limitFields()
  ...
  try {
    ...
    if (!reportInfo) {
      reportInfo = await reportDownloaderService.createReportByUser({
        name: 'Datos Maestros Proveedores',
        code: objectReportResume.code,
        generatorUserId: objectReportResume.generatorUserId,
        reportType: objectReportResume.reportType
      });
    ...
    const nameFile = 'DATOS_MAESTROS_PROVEEDORES';
    const pathTmp = path.resolve(__dirname, '../../resources/uploads/');
    ...
    const csvWriter = createCsvWriter({
      ...
      header: [
        ...
        {
          id: 'tipoNumeroIdentificacionFiscal',
          title: 'Tipo Identificación Fiscal'
        },
        {
          id: 'numeroIdentificacionFiscal',
          title: 'Número Identificación Fiscal'
        },
        {
          id: 'ciudad',
          title: 'Ciudad'
